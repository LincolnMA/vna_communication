...
class vna_driver:
    ...
    end_f = None
    n_points = None
    nmpp  = None # Number of Measurements Per Point
    ...
    def cfg_sweep(self,start,stop,points,n_mean):
        self.start_f = start
        self.end_f = stop
    ...
    def measure(self):
        ...
        total_points = int( self.n_points*self.nmpp )
        step = int( (self.end_f - self.start_f)/self.n_points)
        if total_points <= 255:#max value to one fifo read
            self.sweep(self.start_f,step,self.n_points,self.nmpp)
            return
        #Determina o número de frequênicas diferentes que cabem em um sweep, menos o último 
        n_freqs_per_measure = math.floor(255/self.nmpp)
        #determina o número de frequêncas na última medida
        n_freqs_last_measure = self.n_points%n_freqs_per_measure 
        #determina o número de sweeps, menos o último
        n_measures = math.floor(self.n_points/n_freqs_per_measure)
        #realização das medidas de maneira consecutiva
        for i in range(0,n_measures):
            print("medição ",i)
            self.sweep(int( self.start_f + (i*step*n_freqs_per_measure) ),
                         int( step ),
                         int( n_freqs_per_measure ),
                         int( self.nmpp ))
            #a = input()
        #Última medição
        self.sweep(int( self.start_f + n_measures*step*n_freqs_per_measure ),
                     int( step ),
                     int( n_freqs_last_measure ),
                     int( self.nmpp ))
    
        #ja calibra logo tudo
        if self._cal == True: self.correct_S11()
    ...
    def extract_s11(self):
        if self._s11_ready == False: self.correct_S11()
    ...
    def extract_s11_raw_logmag(self):
        return [self._freqs, to_db(self._S11_RAW)]

    def extract_s11_logmag(self):
        if self._s11_ready == False: self.correct_S11()
    ...
    def extract_smith(self):
        
        if self._cal == False: self.correct_S11()
 ...
 #Não implementado ainda vvvvvvvvvvvvvvvvvvv
    ...
    def calibrate_cli(self):
        
        input("Insert short and press enter...")
        self.measure()
        s11_short = self._S11_RAW
        s21_short = self._S21_RAW

        input("Insert open and press enter...")
        self.measure()
        s11_open = self._S11_RAW
        s21_open = self._S21_RAW

        input("Insert load and press enter...")
        self.measure()
